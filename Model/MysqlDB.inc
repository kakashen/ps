...
class MysqlDB {
    ...
    protected function __construct(string $table) {
        $this->usePool = false;
        $this->table   = $table;
        $this->connect(\Config\Mysql::dsn, \Config\Mysql::username, \Config\Mysql::passwd, \Config\Mysql::options ?? []);
    }
     ...
     * @param array $options
     * @returnb\Connection \Db\Connection
     */
    private function connect($dsn, $user = null, $password = null, $options = array()) {
        if (is_array($dsn)) {
            extract($dsn);
        }
        if ($this->connection) {
            ...
            try {
                ...
                $this->connectionCfg['user']      = $user;
                $this->connectionCfg['password']  = $password;
                $this->connectionCfg['options']   = $options;
    ...
    public function query($sql = null) {
        ...
        if ($this->allowRealExec || $sqlCmd == 'SELECT') {
            ...
            Logger::getInstance()->info($this->connectionCfg['dsn'].":".$trace['class'] . '::' . $trace['method'].":". $this->lastSql.":". $trace['file'] . ':' . $trace['line']);
    ...
    public function exec($sql = null, $releaseConnection = true) {
        ...
        $this->queryBeginTime = microtime(true);
        $trace = $this::getExternalCaller();
        Logger::getInstance()->error($this->connectionCfg['dsn']. '::' . $trace['class'] . '::' . $trace['method']. '::' . $this->lastSql. '::' . $trace['file'] . ':' . $trace['line']);
        ...
        Logger::getInstance()->error($re === false ? 'EXCEPTION' : 'SUCCESS');
        $this->queryEndTime = microtime(true);
        $this->logQuery($sql);
        if (false === $re) {
            // connection broken, retry one time
                    ...
                    if (empty($v_connect)) {
                        if ($this->allowGuessConditionOperator === null || $this->allowGuessConditionOperator === true) {
                            if ($this->allowGuessConditionOperator === null)
                                Logger::getInstance()->error("guessing condition operator is not allowed: use '$k IN'=>array(...) type => ". E_WARNING);
                            $v_connect = 'IN';
                        } else
        ...
        if (is_array($sql)) {
            ...
            foreach ($sql as $k => $v) {
                if (is_int($k))
                    ...
                    Logger::getInstance()->error('unknown key("' . $k . '") in sql.');
            }
        }
        return $realSql;
    }
    ...
    public function queryScalar($sql = null, $default = null) {
        ...
        Logger::getInstance()->info($this->lastStmt ? 'SUCCESS' : 'EXCEPTION'.($this->memoryUsageAfterFetch - $this->memoryUsageBeforeFetch));
        if ($v !== false)
            return $v;
        return $default;
    }
